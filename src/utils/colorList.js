...
function colorHexToName(color) {
  ...
  if (colorUpper.length < 3 || colorUpper.length > 7) {
    return ['#000000', 'Invalid colorUpper: ' + colorUpper, false];
  }
  if (colorUpper.length % 3 == 0) {
    colorUpper = '#' + colorUpper;
  }
  if (colorUpper.length == 4) {
    colorUpper =
      '#' +
      colorUpper.substr(1, 1) +
      colorUpper.substr(1, 1) +
      colorUpper.substr(2, 1) +
      colorUpper.substr(2, 1) +
      colorUpper.substr(3, 1) +
      colorUpper.substr(3, 1);
  }

  let rgb = rgbColor(colorUpper);
  let r = rgb[0];
  let g = rgb[1];
  let b = rgb[2];
  let hsl = hslColor(colorUpper);
  let h = hsl[0],
    s = hsl[1],
    l = hsl[2];
  let ndf1 = 0;
  ...
  for (var i = 0; i < colorList.length; i++) {
    if (color == '#' + colorList[i][0]) {
      return ['#' + colorList[i][0], colorList[i][1], true];
    }

    ndf1 =
      Math.pow(r - colorList[i][2], 2) +
      Math.pow(g - colorList[i][3], 2) +
      Math.pow(b - colorList[i][4], 2);
    ndf2 =
      Math.pow(h - colorList[i][5], 2) +
      Math.pow(s - colorList[i][6], 2) +
      Math.pow(l - colorList[i][7], 2);
    ndf = ndf1 + ndf2 * 2;
    if (df < 0 || df > ndf) {
      df = ndf;
      cl = i;
  ...
  console.log(colorList[cl][0]);

  return cl < 0
    ? ['#000000', 'Invalid Color: ' + color, false]
    : ['#' + colorList[cl][0], colorList[cl][1], false];
}

module.exports = colorHexToName;
